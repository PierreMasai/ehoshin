...
HoshinItemView = React.createClass({
    ...
    render: function () {
        ...
        return React.createElement(
            ...
            React.createElement(
                "a",
                { href: "", onClick: this.changeHoshin },
                data.name
            )
        );
    }
...
renderHoshin = function (hoshin) {
    ...
    items.fetch({
        success: function (result) {
            var labels = _.map(result.models, function (model) {
            ...
            _.map(models, function (model, index) {
                comments.query({ parent: model.attributes.global_id }).fetch({ success: function (result) {
...
renderIndicators = function (hoshin) {
    ...
    _.mapObject(values, function (val, key) {
        container.append('<div class="col-xs-6 col-sm-4 col-md-2">' + '<span>' + key + '</span>' + '<h3>' + val + '</h3>' + '</div>');
    });
};
