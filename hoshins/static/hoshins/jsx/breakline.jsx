...

{
    ...
    Class.extend = function(childPrototype) {
        ...
        if( Surrogate.prototype.hasOwnProperty('__childrenExtend__') &&
            typeof Surrogate.prototype.__childrenExtend__ == 'function' )
        {
    ...
    BreakLine.Ul = React.createClass({
        render: function() {
            return (
                 <span className="tab-20"> { this.props.text } <br /></span>
            );
    ...
    BreakLine.Error = React.createClass({
        render: function() {
            ...
            for(var i = 0; i < this.props.messages.length; i++)
                rows.push(<BreakLine.Ul text={this.props.messages[i]} key={i}/>);

            return (
                 <div className="alert alert-danger" role="alert">
                    <span className="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
                <span className="tab-5"> {this.props.fieldName} :</span><br />
                    { rows }
                </div>
            );
          }
    ...
    BreakLine.Errors = React.createClass({
        render: function() {
            ...
            nodes = nodes.map(function(error) {
                return <BreakLine.Error fieldName={error[0]} messages={error[1]} key={index++}/>
            });
    ...
    BreakLine.formHelper = {
        ...
        saveModel: function(event, success, error) {
            ...
            if(isNew)
                this.state.model.collection.add(this.state.model);
            ...
            this.state.model.save(null, {
                error: function(model, response, options) {
                    ...
                    if(response.status == 400)
                        errors = parent.formatErrors(response.responseJSON);
                    else if(response.status == 0)
                        errors = {"Server" : ["The server is currently unreachable, please try it again later"]};
                ...
                success: function(model, response, options){
                    ...
                    var done =function() {
                        if (isNew){
                            ...
                            if(typeof parent.componentDidMount === "function")
                                parent.componentDidMount();
                        }
                    };

                    if(typeof success === "function")
                        $.when(success(model, response, options)).then(done);
                    else
                        done();

                    if(! _.isUndefined(app.router))
                        app.router.resetIndicators();
                }
            });
        },
    ...
    BreakLine.List = Class.extend({
        ...
        __childrenExtend__ : function() {
            ...
            if(typeof this.FormView !== "undefined") {
                this.FormView.prototype.newModel = function() {return new parent.ItemModel();};
                ...
                this.FormView.prototype.render = function() {
                    ...
                    if(_.isUndefined(this.state) &&
                       typeof this.getInitialState === "function"
                    )
                        this.replaceState(this.getInitialState());

                    if(this.state.model._isOptimistic)
                        return (<div></div>);
                    else
            ...
            if(typeof this.el !== "undefined" && typeof this.ItemView !== "undefined") {
                ...
                this._View = Backbone.View.extend({
                    ...
                    render: function(model) {
                         ReactDOM.render(
                             React.createElement(parent.View, { data: this.collection.models }),
                                                 document.getElementById(parent.el)
                         );
                    }
                });
                ...
                console.warn("A collection defined without viewer")
        },
        fetch: function() {
            var parent = this;
            ...
            var fetchAux = function() {
                ...
                parent.model.fetch({
                    ...
                    parent.fetched = true;
                    if(typeof parent.view !== 'undefined')
                        parent.view.render();
                })
            };
