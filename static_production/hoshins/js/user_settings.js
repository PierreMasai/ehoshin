...
MembershipItemView = React.createClass({
    displayName: 'MembershipItemView',
    ...
    componentDidMount: function () {
        ...
        var parent = this;
        var mode = 'enable';
        if (this.state.model.attributes.member.id == USER.id) mode = 'disable';

        $(DOMNode).find('input[type=checkbox][data-toggle^=toggle]').bootstrapToggle(mode).change(function (ev) {
    ...
    render: function () {
        ...
        return React.createElement(
            ...
            React.createElement(
                'td',
                null,
                React.createElement('input', { defaultChecked: checked, type: 'checkbox', 'data-toggle': 'toggle' })
            ),
            React.createElement(
                'td',
                null,
                React.createElement(
                    'a',
                    { href: '' },
                    React.createElement('span', { className: 'glyphicon glyphicon-remove remove-user' })
                )
            )
        );
    }
...
$("#user-filter select").on("change", function () {
    ...
    if (type == 'All') $("#userList tr:nth-child(n+2)").show();else {
        $("#userList tr:nth-child(n+2)").each(function () {
            var row = $(this);
            var elt = $(row.find('input[type=checkbox][data-toggle^=toggle]'));

            if (elt.prop('checked').toString() === type) row.show();else row.hide();
        });
    }
});
...
$("#user-filter input").on("change paste keyup", function () {
    ...
    $("#userList tr:nth-child(n+2)").each(function () {
        ...
        if (elt_name.indexOf(name) != -1) {
            if (type == 'All' || type == elt_type) row.show();
        } else row.hide();
    });
});
...
userList.doneFetching(function () {
    ...
    $(".typeahead").typeahead({
        ...
        display: 'username',
        templates: {
            empty: ['<div class="empty-message">', 'Unable to find any users that match the current query', '</div>'].join('\n'),
