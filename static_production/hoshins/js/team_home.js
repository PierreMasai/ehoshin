...
HoshinHeaderView = React.createClass({
    displayName: "HoshinHeaderView",

    mixins: [links],
    render: function () {
        return React.createElement(
            "div",
            { className: "row header_links" },
            React.createElement(
                "a",
                { className: "col-sm-offset-4 col-sm-4 text-center", onClick: this.myself, href: "" },
                React.createElement(
                    "h4",
                    null,
                    "Themes"
            ...
            React.createElement(
                "a",
                { className: "col-sm-4 text-right", onClick: this.comments, href: "" },
                React.createElement(
                    "h4",
                    { className: "important" },
                    "Give your comments on the Hoshin overall"
                )
            )
        );
    }
...
ItemHeaderView = React.createClass({
    ...
    render: function () {
        return React.createElement(
            "div",
            null,
            React.createElement(ItemBottom, { model: this.props.data.model.attributes }),
            React.createElement(
                "div",
                { className: "row header_links" },
                React.createElement(
                    "a",
                    { className: "col-sm-4 text-left", onClick: this.parent, href: "" },
                    React.createElement(
                        "h4",
                        null,
                        "Go back to the Hoshin"
                    )
                ),
                React.createElement(
                    "a",
                    { className: "col-sm-4 text-center", onClick: this.myself, href: "" },
                    React.createElement(
                        "h4",
                        null,
                        "Concrete actions"
                ...
                React.createElement(
                    ...
                    React.createElement(
                        "h4",
                        { className: "important" },
                        "Give your comments on the theme"
                    )
                )
            )
        );
...
PriorityHeaderView = React.createClass({
    displayName: "PriorityHeaderView",
    ...
    render: function () {
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    "a",
                    { className: "col-sm-offset-4 col-sm-4 text-right", onClick: this.comments, href: "" },
                    React.createElement(
                        "h4",
                        { className: "important" },
                        "Give your comments on the concrete action"
                    )
                )
            )
        );
...
app.GlobalHeaderView = React.createClass({
    ...
    render: function () {
        ...
        var types_objects = {
            'Hoshin': React.createElement(HoshinHeaderView, { data: {
                    url: this.props.url
                } }),
            'Theme': React.createElement(ItemHeaderView, { data: {
                    url: this.props.url,
                    model: this.props.model
                } }),
            'Concrete action': React.createElement(PriorityHeaderView, { data: {
                    url: this.props.url,
                    model: this.props.model
                } })
        };
...
app.RoutesManager = Backbone.Router.extend({
    ...
    updateHeaderItem: function (object_type, urlSplitted) {
        ...
        $.get(app.getUrlResource(object_type) + '/' + global_id).done(function (attributes) {
            var types_objects = {
                'hoshins': 'Hoshin',
                'items': 'Theme',
            ...
            ReactDOM.render(React.createElement(app.GlobalHeaderView, params), document.getElementById("header_item"));
        });
    },
    ...
    displayHoshin: function (name, path) {
        ...
        if (_.last(pathSplitted) === "") pathSplitted.pop();
        ...
        if (isNaN(_.last(pathSplitted))) throw 'The url is wrong...';
        ...
        if (object_type === 'hoshins' || excelExport.attr("href") == '') {
            excelExport.attr("href", app.getUrlResource('hoshins') + '/' + idHoshin + '?type=application/excel');
        }
        ...
        if (comments) {
            ...
            app.priorityList.doneFetching(function () {
                ...
                if (app.priorityList.model.length == 0 && !(!_.isUndefined(lastUrl) && _.last(lastUrl) == global_id && lastUrl.length == 4)) {
    ...
    redirectHoshin: function (name) {
        ...
        if (_.isUndefined(firstHoshin)) {
            ...
            } else switchSection('noHoshinTextNo');
        } else {
        ...
        if (object_type === 'hoshins' || isEmpty) {
            app.hoshinList.doneFetching(function () {
                ...
                if (model) {
                    ...
                    ReactDOM.render(React.createElement(app.HoshinIndicatorList, model.attributes), document.getElementById("hoshinIndicators"));
                }
            });
        }
    }
...
function switchSection(sectionName) {
    ...
    $.each(sections, function (index, name) {
        if (name === sectionName) $('#' + name).show();else $('#' + name).hide();
    });
}
...
$(function () {
    ...
    if (!_.isUndefined(TEAM_NAME)) showTeamSpace();else console.error("Any team name given in the url... How did you get there?");
});